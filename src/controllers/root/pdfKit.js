...
function drawInvoiceTableHeader(doc, y) {
  ...
  doc.text("Qty", pageWidth - 230, y + 8);
  doc.text("Unit", pageWidth - 160, y + 8);
  doc.text("Total", pageWidth - 100, y + 8);
  ...
  try {
    ...
    if (design === "invoice") {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        doc.text(item.quantity, pageWidth - 210, y);
        ...
        // total
        doc.font(FONT_BOLD);
        doc.text(`Rs. ${total}`, pageWidth - 90, y);
      ...
      totals.forEach(([label, value], i) => {
        ...
        doc
          .font(FONT_BOLD)
          .fillColor(isDiscount ? "#DC2626" : DARK)
